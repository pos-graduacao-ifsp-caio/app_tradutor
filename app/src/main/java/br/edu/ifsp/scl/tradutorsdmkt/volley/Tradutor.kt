...
class Tradutor(val mainActivity: MainActivity) {
                        ...
                        it?.entries?.forEach {
                            it?.senses?.forEach {
                                it?.translations?.forEach {
                                    ...
                                    Log.v(
                                        mainActivity.getString(R.string.app_name),
                                        "Nome da classe Java: ${ clTransK.qualifiedName }"
                                    )
                                    ...
                                    clTransK.declaredMemberProperties.forEach {
                                        Log.v(
                                            mainActivity.getString(R.string.app_name),
                                            "Nome e tipo do atributo Kotlin: ${ it.name } ${ it.returnType }"
                                        )
                                    }
                                    ...
                                    Log.v(
                                        mainActivity.getString(R.string.app_name),
                                        "Invocando método Java: ${ getTextJ.invoke(objTransJ) } "
                                    )

                                    Log.v(
                                        mainActivity.getString(R.string.app_name),
                                        "Invocando função Kotlin: ${ getTextK.call(objTransK) } "
                                    )
    ...
    class TranslationListDeserializer : JsonDeserializer<List<Translation>> {
        ...
        override fun deserialize(json: JsonElement?, typeOfT: Type?, context: JsonDeserializationContext?): List<Translation>{
            ...
            lexicalEntries?.forEach {
                it?.let {entries.addAll(it.asJsonObject?.getAsJsonArray("entries"))
                }
            }
            ...
            entries.forEach {
                it?.let {senses.addAll(it.asJsonObject?.getAsJsonArray("senses"))
                }
            }

            /* Juntando todos os JsonArrays “translations” de todos “senses” num só JsonArray,    O(N^2) */
            val translations: JsonArray = JsonArray()
            senses.forEach {
                it?.let {translations.addAll(it.asJsonObject?.getAsJsonArray("translations"))
                }
            }
            ...
            translations.forEach {
                it?.let {val translation: Translation = Translation()
    ...
    inner class RespostaListener : Response.Listener<JSONObject> {
        override fun onResponse(response: JSONObject?) {
            try{
                ...
                /* Usa o builder para criar um Gson e usa o Gson para converter o Json de respostanuma lista de Translation usando o desserializador personalizado. */
                val listTranslation: List<Translation> =gsonBuilder.create().fromJson(response.toString(), listTranslationType)

                // Extrai somente o texto dos objetos Translation
                val listTranslationString: StringBuffer = StringBuffer()
                listTranslation.forEach {
                    listTranslationString.append("${it.text}, ")
                }
    ...
    }           esse respostaListener é mais eficiente que o outro    */
